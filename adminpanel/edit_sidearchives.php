...
if (isset($_POST['sidearchivi_save'])){
    $title = $_POST['title'];
    $theme = $_POST['date'];
    $sidearchiverow_id = $_POST['sidearchivirow_id'];

    $DB ->query("UPDATE  sidearchive SET  title = ?, side_date =? WHERE id = ?", array("$title", "$theme", "$sidearchiverow_id"));

    echo "Succes information!!";

    echo '<script>window.location.href = "side_archivies.php"</script>';
    //header("location:side_archivies.php");
}
...
<div class="page-wrapper">
    ...
    <div class="topbar">
        ...
        <nav class="navbar-custom text-center">
            <h2 class="p-3">Внести изменение sidearchivies</h2>
        </nav>
        <!-- end navbar-->
    </div>
    ...
    <div class="container">
        <?php
        $result = $DB ->query("SELECT * FROM sidearchive WHERE id = ?", array("$id"));
            ...
            <form class="mt-5" method="post">
                <div class="form-row">
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <input type="date" name="date" value="<?php echo $row["side_date"];?>" class="form-control" id="validationDefault01" >
                    </div>
                </div>
                <input type="hidden" name="sidearchivirow_id" value="<?php echo $row["id"];?>">
                <a href="side_archivies.php"  class="btn btn-danger" type="submit">Назад</a>
                <button class="btn btn-primary"  type="submit" name="sidearchivi_save">Сохранить</button>
            </form>
        <?php endforeach;?>
