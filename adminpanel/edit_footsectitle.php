...
if (isset($_POST['footsecond_save'])){
    ...
    $title = $_POST['title'];
    $footsecond_row_id = $_POST['footsecond_row_id'];

    $DB ->query("UPDATE  footer_secondtitle SET  article = ?, title = ? WHERE id = ?", array("$article", "$title", "$footsecond_row_id"));
...
<div class="page-wrapper">
    ...
    <div class="topbar">
        ...
        <nav class="navbar-custom text-center">
            <h2 class="p-3">Внести изменение footersecond</h2>
        </nav>
        <!-- end navbar-->
    </div>
    ...
    <div class="container">
        <?php
        $result = $DB ->query("SELECT * FROM footer_secondtitle WHERE id = ?", array("$id"));
        foreach ($result as $row): ?>
            <form class="mt-5" method="post">
                <div class="form-row">
                    <div class="col-md-6 mb-3">
                        <label for="validationDefault01">Название стати</label>
                        <input type="text" name="article" value="<?php echo $row["article"];?>" class="form-control" id="validationDefault01" >
                    </div>
                    <div class="col-md-6 mb-3">
                ...
                <input type="hidden" name="footsecond_row_id" value="<?php echo $row["id"];?>">
